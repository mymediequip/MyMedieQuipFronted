...
export const Checkout = () => {
  ...
  const inspectionStatus = useSelector(
    (state) => state.profileData.inspection_status
  );
  ...
  return (
    <div className={styles.checkoutCont}>
      <div className={styles.checkoutData}>
        <PayLogin inspection={inspectionStatus} />
        <DelieveryAddress
      ...
      {!inspectionStatus ? (
        ...
        <div className={styles.checkoutPriceData}>
          ...
          <div>
            <b>Token Amount :</b>
            <b>₹{price ?( price * 10 / 100) : "10000"}</b>
          </div>
          <div style={{ marginTop: "2rem" }}>
            <p>(Note : The Token amout will be refunded xxxxxxxxxxx)</p>
          </div>
                  ...
                  <div className={styles.addRadio}>
                    <div>
                      <input
                        ...
                        value="home"
                        onChange={formik.handleChange}
                        checked={formik.values.delivery_add == "home"}
                      />
                    ...
                    <div>
                      <input
                        ...
                        value="work"
                        onChange={formik.handleChange}
                        checked={formik.values.delivery_add == "work"}
                      />
...
const OrderSummary = ({ details, profileDetails, inspection }) => {
...
const OrderSummaryCard = ({ details, profileDetails, insp }) => {
  const price = useSelector((state) => state.profileData.eqip_price_update);
      ...
      <div className={styles.orderProdSub}>
        <div className={styles.orderText}>
          ...
          <div className={styles.orderLoc}>
            <img src={location} width="17px" style={{marginRight : "5px"}} alt="..." />
            <span>{profileDetails?.location}</span>
          </div>
         </div>
       ...
       <div className={styles.orderAmt}>
        <h3 className={styles.orderAmt1}>Total Amount : {Number(price ? price : details?.asking_price).toFixed(2)} </h3>
        <h3>Token Amount : {Number(price ? (price*10 /100)  : details?.asking_price * 10 / 100).toFixed(2)} </h3>
       </div>
        </div>
      </div>)
      }
        ...
        <div className={styles.payOptions}>
          {
            paymentType.sort().map((type)=>{
                  return(
                    <PaymentOptCard 
                    data={{ 
                      name:  type.name == "EMI" ? `${type.name} (Easily Installment)` : type.name, 
                      img: type?.id  == 1 ? paytam 
                       : type?.id == 2 ?  upib  : "",
                      tag : type?.id == 3 ? "Add and secure your card as per RBI guidelines"
                      : type?.id  == 4 ? "This instrument has low success, use UPI or cards for better experience" : ""
                      }}
                     />
                  )
            })
          ...
          <PaymentOptCard
            data={{
              ...
              tag: "Add and secure your card as per RBI guidelines",
            }}
          />
          <PaymentOptCard
            data={{
              name: "Net Banking",
              tag: "This instrument has low success, use UPI or cards for better experience",
            }}
          />
